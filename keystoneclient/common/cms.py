...
def pkiz_sign(text,
    ...
    #                        PKIZ_CMS_FORM)
    #sazzad adds
    hashed_token = cms_hash_token(text, mode='md5')
    #change text to hashed token Sazzad
    signed = cms_sign_data(hashed_token,
                           signing_cert_file_name,
                           signing_key_file_name,
                           PKIZ_CMS_FORM)
...
def pkiz_uncompress(signed_text):
    ...
    #Sazzad adds
    hashed_token = signed_text[len(PKIZ_PREFIX):].encode('utf-8')
    # print("in uncompress$$$$$$$$$$$$$$$$$$$$$$$$$$$$" ,hashed_token)
    #sazzad changed text to hashed_token
    unencoded = base64.urlsafe_b64decode(hashed_token)
...
def cms_hash_data(text, mode='md5'):
    ...
    return: for plaintext tokens, returns the hash of the passed in token
            otherwise, returns what it was passed in.
    """
    hasher = hashlib.new(mode)
