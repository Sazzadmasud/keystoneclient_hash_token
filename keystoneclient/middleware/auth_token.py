...
class AuthProtocol(object):
    ...
    def _get_user_token_from_header(self, env):
        ...
        if token:
            #sazzad adds
            # load the contents of the token as a JSON formatted string
            tokenData = json.loads(token,object_pairs_hook=ordereddict.OrderedDict)
    ...
    def _validate_user_token(self, user_token, env, retry=True):
        ...
        try:
            ...
            if cached:
                ...
                data = jsonutils.loads(verified)
                #sazzad added below
            elif cms.cms_plaintext_token(user_token):
                #sazzad
                token = json.loads(user_token, object_pairs_hook=ordereddict.OrderedDict)
                token_id = token['access']['token']['id']
                #check if it is plaintext token
                token_hash_formatted = cms.token_to_cms(token_id)
                verfied_token_hash = jsonutils.loads(self.cms_verify(token_hash_formatted))
                # verified_token_hash is in unicode format
                # Replace the contents of the ID with the 'placeholder' keyword.
                received_token_hash = cms.cms_hash_data(cms.cms_replace_id(token))
                # verfied_token_hash.encode("utf-8") makes it a string.
                if received_token_hash == verfied_token_hash.encode("utf-8"):
                    data=json.loads(cms.cms_replace_id(token))
                else:
                    raise Exception
            else:
